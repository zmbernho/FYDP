...
var xPos = 0;
var yPos = 0;
var yRangeMin = document.currentScript.getAttribute('yMin'); 
var yRangeMax = document.currentScript.getAttribute('yMax');
...
function gameloop()
{
	...
	//have x,y
	//have range
	var yBoxMin = center-200;
	var yBoxMax = center+135;
	var yVal=0;
	var hnotes=0;
	...
	{
		if (hnotes == audiogame.dots[i].line && Math.abs(audiogame.dots[i].distance) < 20)
		{
	...
	var gSect=(yRangeMax-yRangeMin)/5;

	if (yPos <= gSect) {
		yVal = center+175-37;
		hnotes=5;
	} else if (yPos <= (2*gSect) && yPos > gSect) {
		yVal = center+100-37;
		hnotes=4;
	} else if (yPos <= (3*gSect) && yPos > (2*gSect)) {
		yVal = center+25-37;
		hnotes=3;
	} else if (yPos <= (4*gSect) && yPos > (3*gSect)) {
		yVal = center-50-37;
		hnotes=2;
	} else {
		yVal = center-125-37;
